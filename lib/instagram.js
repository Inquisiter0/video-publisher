...
export async function createReelContainer({ igUserId, accessToken, videoUrl, caption }) {
  ...
  console.log('[IG publish] Creating container for igUserId:', igUserId, 'videoUrl:', videoUrl);
  const res = await fetch(`${GRAPH_API_BASE}/${igUserId}/media?${params}`, { method: 'POST' });
  const text = await res.text();
  console.log('[IG publish] Container creation response:', res.status, text);
  if (!res.ok) {
    throw new Error(`IG container creation failed: ${res.status} ${text}`);
  }

  const data = JSON.parse(text);
  console.log('[IG publish] Container ID:', data.id);
  return data.id;
}
